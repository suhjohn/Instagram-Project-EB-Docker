...
class FrontFacebookLogin(View):
    """
    {'access token' : 액세스 토큰값, 'facebook_user_id' : <유저의 app에 대한 페북 ID 고유값>}
    """
    class AccessTokenInfo(NamedTuple):
        access_token: str
        token_type: str
    ...
    def get(self, request):
        ...
        def get_access_token_info(code):
            ...
            response = request.get(url_access_token, params_access_token)
        ...
        def get_debug_token_info(token):
            ...
            response = request.get(url_debug_token, params_data)
